...
#include "lht_redis_i.h"
#include "lht_redis_core.h"

class QTimer;
class LHNodeInfo;
class ctkPluginContext;
class LHLoggerInterface;
class LHSettingsInterface;
...
class LHRedis : public LHRedisInterface
{
    Q_OBJECT
    Q_INTERFACES(LHRedisInterface)
public:
    LHRedis();
    ~LHRedis();

    virtual int Init(const LHParameters &Parameters);
    virtual int Uninit();
    virtual int CreateInstance(QVariant &varInstance, const LHParameters &Parameters);
    virtual int DestroyInstance(const QVariant &varInstance);
...
public:
    ...
    */
     ...
     */
    virtual QStringList set_randmember(QString key, bool *error = NULL) = 0;
...
private:
    ...
    QVariant excuteCmd(FuncType type, LHRedisCore* redis, QVariantList &data);

private:

    bool                    m_bInit;
    int                     m_nInstance;
    int                     m_nSettingsInstance;
    ...
    LHSettingsInterface    *m_Settings;
    LHLoggerInterface      *m_Log4qt;
